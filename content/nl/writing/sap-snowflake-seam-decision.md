---
title: Bepalen wat er over de SAP↔Snowflake-grens gaat
summary: Voor elk object in een SAP-landschap bepaalt één vraag de kosten en de versheid van alles wat erop volgt. Zero-copy delen, repliceren, federeren, splitsen, of in SAP houden. Dit is een referentiearchitectuur die die vraag per object beantwoordt, met negen geordende regels, een kostenmodel met de omslagfrequentie, en een lokale simulatie die alle drie de modi draait.
date: 2026-08-10
order: 1
---

# Bepalen wat er over de SAP↔Snowflake-grens gaat
...
## Vijf manieren over de grens

Alles links van de integratielaag hoort bij SAP; alles rechts ervan hoort bij Snowflake. De beslissingen die ertoe doen zitten op de grens ertussen, en er zijn vijf uitkomsten om uit te kiezen:
...
Het diagram loopt bovendien beide kanten op. Snowflake dient ook als bron voor Datasphere, als remote table of binnen een replicatieflow, en zo werken de meeste echte landschappen.
...
2. **Deltacapaciteit.** Replicatie past bij objecten die een delta kunnen leveren. Boven het omvangsplafond valt de modus af op een volledige herlaad per cyclus, wat de gebruikelijke uitkomst is voor custom Z-tabellen.
...
4. **Latency-SLO bij piekconcurrency.** Federatie draait op het systeem dat ook de business draait, en wordt daarom gemeten bij de maandafsluiting, niet op een rustige middag.
...
8. **Hybride.** Splits het object waar het twee workloads is.
...
Economie op plek zeven houden is hier de architecturale toezegging, en de code dwingt het af. Constraints elimineren voordat er ook maar één kostenpost berekend wordt, zodat een goedkope optie zich nooit langs een residentieregel kan praten.
...
Hier net zo duidelijk gezegd als in de README. DuckDB staat aan beide kanten van de grens model. De kostencijfers zijn illustratieve placeholders in de vorm van lijstprijzen, ondergebracht in één YAML-bestand zodat een echte rate card er zo in past. De catalogus van 24 objecten is synthetisch, gevormd naar een DACH-maakbedrijf. Het integratielandschap van SAP beweegt snel, dus de genoemde mechanismen zijn het waard om tegen actuele documentatie te controleren voordat er een budget van afhangt.

Het duurzame artefact is de beslisprocedure. Laad een echte rate card, draai opnieuw, en lees wat er verschuift. Elke beslissing die onder echte prijzen beweegt, was een economische keuze en nooit een architecturale, en dat is nuttig om vroeg over je eigen landschap te weten.

## Waarom ik zo bouw

Dit is de werkwijze die ik overal hanteer: intentie eerst opgeschreven, afwegingen vastgelegd als ADR's, en de claim aangetoond in plaats van beweerd. Ik heb deze grens in het echt geleverd; zie de casus [SAP S/4HANA Finance → Snowflake](/nl/work/sap-snowflake), waar datacontracten precies daar zitten waar SAP overdraagt aan het lakehouse. Het patroon is op beide plekken hetzelfde. Leg het ontwerpvlak op de grens, en maak de beslissing daar expliciet genoeg om te reviewen.
...
*De repo is openbaar en MIT-gelicentieerd: [github.com/fps4/sap-bdc-snowflake-blueprint](https://github.com/fps4/sap-bdc-snowflake-blueprint). Begin bij `docs/reference-architecture.md`, de pagina die op het scherm hoort; al het andere bestaat om die te ondersteunen.*
