...
summary: For every object in a SAP estate, one question sets the cost and the freshness of everything downstream. Share it zero-copy, replicate it, federate it, split it, or keep it in SAP. Here is a reference architecture that answers it per object, with nine ordered rules, a cost model with the crossover frequency, and a local simulation that runs all three modes.
date: 2026-08-10
order: 1
---
...
The diagram also runs in both directions. Snowflake serves as a source for Datasphere, as a remote table or inside a replication flow, which is how most real estates work.
...
4. **Latency SLO at peak concurrency.** Federation runs on the system that also runs the business, so it is measured at month-end close, not on a quiet afternoon.
...
8. **Hybrid.** Split the object where it is two workloads.
...
The durable artifact is the decision procedure. Load a real rate card, re-run, and read what moves. Any decision that shifts under real prices was an economic call and never an architectural one, which is a useful thing to learn about your own estate early.
...
*The repo is public and MIT-licensed: [github.com/fps4/sap-bdc-snowflake-blueprint](https://github.com/fps4/sap-bdc-snowflake-blueprint). Start with `docs/reference-architecture.md`, the page to put on a screen; everything else exists to support it.*
