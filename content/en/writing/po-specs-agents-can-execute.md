...
For most teams the answer is the AI team, and that's the problem. Product owners are surrounded by AI, expected to ship features that use it, and almost never taught the one skill that moves the needle: directing it. The way a senior engineer directs a junior, with a spec clear enough to execute against.
...
Same agent, same model. The second output is dramatically better, and the model did not get any smarter in between. The spec removed the guesswork. The agent didn't have to decide what "export" meant, because the PO already had.

That's the whole move. The intelligence didn't change. The direction did.

## The three things a PO needs

Once a PO sees that, the rest follows. I teach three skills, and the first carries most of the weight:

1. **Directing AI like a senior engineer would.** Writing a spec an agent can execute: decomposing the work, stating acceptance criteria in testable form, naming what's out of scope. A prompt is a wish. A spec is an instruction.
...
None of this is a training gimmick. It's how I run delivery with agents. In the `maestro` reference architecture, agents don't merge anything. They propose, and a human disposes, behind functional and technical gates. The spec is the contract. The evals are the sign-off. Branch protection enforces "humans decide".

A PO who writes specs an agent can execute is operating that same loop at the product level: clear intent in, evaluated output out, a human in control of the gate. It's a force multiplier. One person whose specs make the whole team's agents more effective.
