...
export const repos: Repo[] = [
  ...
  },
  {
    ...
    proves: {
      en: 'Microsoft Fabric, Databricks-on-Azure or Snowflake-on-Azure for a 20-workload Synapse estate, decided per workload rather than per feature. Ten ordered rules where residency, T-SQL surface, streaming semantics and ML lifecycle eliminate, and cost only chooses among what survives. The structural point is that a Fabric workload has no standalone price: one pre-paid F-SKU carries the estate, so one more workload costs nothing until the capacity saturates and then costs a whole rung — which is why two notebooks end up metered on Databricks solely because they would have stepped F32 to F64, and why unused headroom turns out to be the cheapest compute in the estate. A second meter adds the mirror problem: a platform costs money to operate whether it is busy or not, so R9 checks after the fact whether each one opened on economics alone actually covered its own overhead. The sharpest result is the empty column — Snowflake is the cheapest meter on five of the twenty workloads and R2 eliminates it from four of them, because they are written in T-SQL. It prices best exactly where it is not allowed to compete, which is a conclusion a capability matrix cannot reach (no prices) and a price table cannot either (no constraints). Two eliminations are executed rather than asserted: the same payment stream finishes 3.49% out per window on an ingestion-time engine and exactly right on an event-time one, and a load killed between its two writes leaves 893 torn rows once its multi-table transaction is taken away. One workload has no home at all, and the register says which single fact has to change. Runs with `make demo` — no Azure subscription, no capacity, no Docker.',
      nl: 'Microsoft Fabric, Databricks-op-Azure of Snowflake-op-Azure voor een Synapse-landschap van 20 workloads, beslist per workload in plaats van per feature. Tien geordende regels waarbij dataresidentie, T-SQL-oppervlak, streamingsemantiek en ML-levenscyclus elimineren, en kosten alleen kiezen uit wat overblijft. Het structurele punt: een Fabric-workload heeft geen eigen prijs. Eén vooruitbetaalde F-SKU draagt het hele landschap, dus een extra workload kost niets tot de capaciteit vol is en daarna een hele trede — en dat is waarom twee notebooks op Databricks gemeterd eindigen, puur omdat ze F32 naar F64 zouden duwen, en waarom ongebruikte ruimte de goedkoopste rekenkracht in het landschap blijkt. Een tweede meter voegt het spiegelprobleem toe: een platform kost geld om te beheren of het nu druk is of niet, dus R9 toetst achteraf of elk platform dat puur op economische gronden is geopend zijn eigen overhead heeft terugverdiend. Het scherpste resultaat is de lege kolom — Snowflake is de goedkoopste meter op vijf van de twintig workloads en R2 sluit het uit van vier daarvan, omdat ze in T-SQL geschreven zijn. Het heeft precies daar de beste prijs waar het niet mee mag doen, een conclusie die een capability matrix niet kan bereiken (geen prijzen) en een prijstabel evenmin (geen constraints). Twee eliminaties worden uitgevoerd in plaats van beweerd: dezelfde betaalstroom komt op een ingestion-time-engine 3,49% per venster verkeerd uit en op een event-time-engine precies goed, en een load die tussen zijn twee schrijfacties wordt afgebroken laat 893 inconsistente rijen achter zodra je de multi-table-transactie weghaalt. Eén workload heeft helemaal geen bestemming, en het register benoemt welk enkele feit daarvoor moet veranderen. Draait met `make demo` — geen Azure-abonnement, geen capaciteit, geen Docker.',
    },
  },
